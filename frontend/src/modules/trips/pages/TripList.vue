<template>
  <div class="quote-list-wrapper">
    ...
    <div class="page-header">
      <h3 class="page-title">TRIP MANAGEMENT</h3>
            ...
            <table class="table table-sm table-hover" >
              <thead>
                <tr>
                  ...
                  <th>Dep.&nbsp;Date</th><th>Dep.&nbsp;City</th><th>User</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="quote in quotes" :key="quote.id">
                  ...
                  <td>
                    <router-link :to="{ name: 'TripManagement', params: { id: quote.id } }"
                                 class="btn btn-sm btn-outline-primary me-1">
                      <i class="fa fa-cogs" /> Manage
                    </router-link>
...
const currentStatus = ref(route.query.status ?? 'accepted')
...
async function fetchQuotes() {
  ...
  try {
    const params = {
      ...
      page: pageInfo.page,
      q: search.value,
      status: currentStatus.value  // Always include status filter
    }
...
watch(
  () => route.query,
  q => {
    currentStatus.value = q.status ?? 'accepted'
